...
result = open("/scratch/pjohri1/MSMC_SIMS/genome10/sfs/" + demo + "_" + dfe + "_rep" + str(rep) + "_masked_" + str(num_indv) + ".sfs", 'w+')
l_af = []
chr_num = 1
while chr_num <= 22:
    print ("chr number is: " + str(chr_num))
    d_af = {}
    f_ms = open("/scratch/pjohri1/MSMC_SIMS/genome10/" + demo + "/" + dfe + "/output_genome" + str(rep) + "_chr" + str(chr_num) + "_masked.ms", 'r')
    linecount = 0
    for line in f_ms:
        line1 = line.strip('\n')
