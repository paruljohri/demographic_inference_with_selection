    ...
    for model in models:
        for rep in range(1, number_of_fsc_replicates + 1):
            if simulation_tool == 'msprime':
                fsc_results_file = '$$$/Fastsimcoal2_Chr_Segments_OutputFiles/msprime/' + model + '_' + SNP_density + '_' + chr_size + '_rep' + str(rep) + '.bestlhoods'
            elif simulation_tool == 'slim':
                fsc_results_file = '$$$/Fastsimcoal2_Chr_Segments_OutputFiles/slim/' + model + '_' + SNP_density + '_' + chr_size + '_rep' + str(rep) + '.bestlhoods'
            ...
            if model == 'inst_bot':
                replicate_NCurrent_value = float(parameter_values[parameter_titles.index('NCurrent$')])
                replicate_NBot_value = float(parameter_values[parameter_titles.index('NBot$')])
                replicate_Time_value = float(parameter_values[parameter_titles.index('Time$')])
                replicate_MaxEstLhood_value = float(parameter_values[parameter_titles.index('MaxEstLhood')]) 
...
GRAPH_SNP_density = ['all','thinned_5kb','thinned_50kb','thinned_100kb'] #Specify the SNP density

simulation_tool = 'slim' #Specify either 'slim' or 'msprime' for which results will be analyzed

display_graph = False #Specify either 'True' or 'False' for whther or not to display the plots
