...
def demographic_model_parameters(demographic_model, exon_density):
    ...
    if '2fold' in exon_density:
        ...
        if demographic_model == 'decline':
            min_viewable_population_size = 100
            max_viewable_population_size = 50000
#             min_viewable_population_size = 1
#             max_viewable_population_size = 10
            max_plotted_generation = 100000 
            for generation in range(0, max_plotted_generation):
                true_generations.append(generation)
                if generation < 4752:
...
def analyze_msmc_results(number_of_msmc_replicates, max_plotted_generation, bin_size, demographic_model, masking_state, exon_density, DFE_number, mutation_rate):         
    ...
    for k in range(1, number_of_msmc_replicates + 1):
        msmc_results_file = '$$$/MSMC_Human_Genome_OutputFiles/fixed_rec_and_mut/RAW_rep' + str(k) + '_' + demographic_model + '_sim' + str(DFE_number) + '_' + exon_density + '_' + masking_state + '.final.txt'
    ...
    for model in models:
        for rep in range(1, number_of_fsc_replicates + 1):
            ...
            for trial in range(1, number_of_fsc_trials_per_replicate + 1):
                fsc_results_file = '$$$/Fastsimcoal2_Human_Genome_OutputFiles/fixed_rec_and_mut/' + model + '_' + SNP_density + '_' + demographic_model + '_' + masking_state + '_' + exon_density + '_sim' + str(DFE_number) + '_rep' + str(rep) + '_trial' + str(trial) + '.bestlhoods'
...
mutation_rate = 10**-8 #Specify the mutation rate the genomes were simulated under for the purposes of converting the raw MSMC output values 
